...
#if defined(_WIN64) || defined(__WIN32__) || defined(_WIN32) || defined(WIN32) || defined(__WINDOWS__) ||              \
    defined(__TOS_WIN__) || defined(__CYGWIN__)
#ifndef CE_WINDOWS
#define CE_WINDOWS
#endif
...
class ceDateTime {
  ...
  public:
    ...
    // output: Julian date
    static double w2j(long year, long month, long day, long hour = 12, long minute = 0, double second = 0, long ct = 0,
                      long SG = 2361222);
    ...
    static void j2w(double jd, long &year, long &month, long &day, long &hour, long &minute, double &second,
                    long ct = 0, long SG = 2361222);
    ...
    static std::string j2s(double jd, std::string fm = "%Www %y-%mm-%dd %HH:%nn:%ss %zz", double tz = 0, long ct = 0,
                           long SG = 2361222);
    ...
    void SetDateTime(long year, long month, long day, long hour = 12, long minute = 0, double second = 0, double tz = 0,
                     long ct = 0, long SG = 2361222);
...
// SG: Beginning of Gregorian calendar in JDN [Optional argument: (default=2361222)])
// output: Western date (y=year, m=month, d=day, h=hour, n=minute, s=second)
inline void ceDateTime::j2w(double jd, long &year, long &month, long &day, long &hour, long &minute, double &second,
                            long ct, long SG) {
    long j, y, m, d, h, n;
    double jf, s;
    if (ct == 2 || (ct == 0 && (jd < SG))) {
        ...
        jf = jd + 0.5 - j;
        b = j + 1524;
        c = long(floor((double(b) - 122.1) / 365.25));
        f = long(floor(365.25 * double(c)));
        e = long(floor(double(b - f) / 30.6001));
        m = (e > 13) ? (e - 13) : (e - 1);
        d = b - f - long(floor(30.6001 * double(e)));
        y = m < 3 ? (c - 4715) : (c - 4716);
    } else {
        j = long(floor(jd + 0.5));
        jf = jd + 0.5 - j;
...
//-------------------------------------------------------------------------
// julian date to unix time
inline time_t ceDateTime::j2u(double jd) { return long(floor((jd - 2440587.5) * 86400.0 + 0.5)); }
//-------------------------------------------------------------------------
// Compile time string
...
//-------------------------------------------------------------------------
// set local time zone
inline void ceDateTime::SetTimezone() { this->m_tz = ceDateTime::ltzoh(); }
//-------------------------------------------------------------------------
// set time to now
inline void ceDateTime::Set2Now() { this->m_jd = ceDateTime::jdnow(); }
//-------------------------------------------------------------------------
#endif // CE_NOS
...
inline std::string ceDateTime::j2s(double jd, std::string fm, double tz, long ct, long SG) {
    ...
    std::string M[] = {"January", "February", "March",     "April",   "May",      "June",
...
inline double ceDateTime::s2j(std::string tstr, double tz, long ct, long SG) {
    ...
    if (str.length() == 8 || str.length() == 14 || str.length() == 17) {
        pstr = str.substr(0, 4);
        y = stol(pstr); // get year
        pstr = str.substr(4, 2);
        m = stol(pstr); // get month
        pstr = str.substr(6, 2);
        d = stol(pstr); // get day
        if (str.length() == 14 || str.length() == 17) {
            pstr = str.substr(8, 2);
            h = stol(pstr); // get hour
            pstr = str.substr(10, 2);
            n = stol(pstr); // get minute
            pstr = str.substr(12, 2);
            s = double(stol(pstr)); // get second
            if (str.length() == 17) {
                pstr = str.substr(14, 3);
                ls = double(stol(pstr)); // get millisecond
                s += ls / 1000.0;
            }
        }
...
//-------------------------------------------------------------------------
// set time in jd
inline void ceDateTime::SetJD(double jd) { this->m_jd = jd; }
//-------------------------------------------------------------------------
// set in unix time
inline void ceDateTime::SetUnixTime(time_t ut) { this->m_jd = ceDateTime::u2j(ut); }
//-------------------------------------------------------------------------
// set date time for a timezone and a calendar type
inline void ceDateTime::SetDateTime(long year, long month, long day, long hour, long minute, double second, double tz,
                                    long ct, long SG) {
    this->m_jd = ceDateTime::w2j(year, month, day, hour, minute, second, ct, SG) - tz / 24.0;
}
//-------------------------------------------------------------------------
// set calendar type [0=British (default), 1=Gregorian, 2=Julian]
inline void ceDateTime::SetCT(long ct) { this->m_ct = ct % 3; }
//-------------------------------------------------------------------------
// set Beginning of Gregorian calendar in JDN [default=2361222]
inline void ceDateTime::SetSG(long sg) { this->m_SG = sg; }
//-------------------------------------------------------------------------
// set time using a date time string
// inputs
//  tstr : time string
