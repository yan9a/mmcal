...
class ceMmDateTime : public ceDateTime {
  ...
  public:
    ...
    // Ref:
    // https://my.wikipedia.org/wiki/%E1%80%9E%E1%80%AC%E1%80%9E%E1%80%94%E1%80%AC_%E1%80%9E%E1%80%80%E1%80%B9%E1%80%80%E1%80%9B%E1%80%AC%E1%80%87%E1%80%BA
    ...
    // This update offers flexibility in defining the Sasana year which is preferable to enforcing a single fixed
    // approach.
...
inline void ceMmDateTime::GetMyConst(long my, double &EI, double &WO, double &NM, long &EW) {
    ...
    // Makaranta system 2 (ME 798 - 1099)
        ...
        WO = -1.1;
        NM = -1;
        long fme12[][2] = {{813, -1}, {849, -1}, {851, -1}, {854, -1}, {927, -1}, {933, -1}, {936, -1},
    ...
    i = ceMmDateTime::bSearch2(my, fme, uf);
    if (i >= 0)
        WO += fme[i][1]; // full moon day offset exceptions
    i = ceMmDateTime::bSearch1(my, wte, uw);
    if (i >= 0)
...
inline void ceMmDateTime::cal_my(long my, long &myt, long &tg1, long &fm, long &werr) {
    ...
    werr = 0;
    ceMmDateTime::cal_watat(my, y2watat, y2fm);
    myt = y2watat;
    do {
        yd++;
        ceMmDateTime::cal_watat(my - yd, y1watat, y1fm);
    } while (y1watat == 0 && yd < 3);
    if (myt) {
        nd = (y2fm - y1fm) % 354;
        myt = long(floor(nd / 31) + 1);
        fm = y2fm;
        if (nd != 30 && nd != 31) {
            werr = 1;
        }
...
inline void ceMmDateTime::j2m(double jd, long &myt, long &my, long &mm, long &md) {
    ...
    b = long(floor(myt / 2));
    c = long(floor(1 / (myt + 1)));    // big wa and common yr
    myl = 354 + (1 - c) * 30 + b;      // year length
    mmt = long(floor((dd - 1) / myl)); // month type: late =1 or early = 0
    dd -= mmt * myl;
    a = long(floor((dd + 423) / 512));                            // adjust day count and threshold
    mm = long(floor((dd - b * a + c * a * 30 + 29.26) / 29.544)); // month
    e = long(floor((mm + 12) / 16));
    f = long(floor((mm + 11) / 16));
...
inline long ceMmDateTime::cal_mml(long mm, long myt) {
    ...
    if (mm == 3)
        mml += long(floor(myt / 2)); // adjust if Nayon in big watat
    return mml;
}
//-------------------------------------------------------------------------
// Get the apparent length of the year from year type.
...
// input: ( md= day of the month [1-30])
// output: (mf= fortnight day [1 to 15])
inline long ceMmDateTime::cal_mf(long md) { return (md - 15 * long(floor(md / 16))); }
//-------------------------------------------------------------------------
// Get day of month from fortnight day, moon phase, and length of the month
// input: (
//   mf = fortnight day [1 to 15],
...
inline long ceMmDateTime::m2j(long my, long mm, long md) {
    ...
    ceMmDateTime::cal_my(my, myt, tg1, fm, werr); // check year
    mmt = long(floor(mm / 13));
    mm = mm % 13 + mmt; // to 1-12 with month type
    b = long(floor(myt / 2));
    c = 1 - long(floor((myt + 1) / 2));                                      // if big watat and common year
    mm += 4 - long(floor((mm + 15) / 16)) * 4 + long(floor((mm + 12) / 16)); // adjust month
    dd = md + long(floor(29.544 * mm - 29.26)) - c * long(floor((mm + 11) / 16)) * 30 + b * long(floor((mm + 12) / 16));
    myl = 354 + (1 - c) * 30 + b;
...
inline long ceMmDateTime::my2sy(long my, long mm, long md, long k) {
    ...
    if (k == 1) {
        if (mm >= 13)
            buddhistEraOffset = 1183; // if late Tagu or late Kason, then Sasana year is next year
    } else if (k == 2) {
        if ((mm == 1) || (mm == 2 && md < 15))
            buddhistEraOffset = 1181; // if before Kason full moon, then previous year
    }
...
inline long ceMmDateTime::cal_mahayatkyan(long mm, long md) {
    ...
    long mf = md - 15 * (long)floor(md / 16); // get fortnight day [0-15]
    long mahayatkyan = 0;
    long m1 = ((long)floor((mm % 12) / 2) + 4) % 6 + 1;
    if (mf == m1)
        mahayatkyan = 1;
    return mahayatkyan;
}
...
inline long ceMmDateTime::cal_shanyat(long mm, long md) {
    ...
    long mf = md - 15 * (long)floor(md / 16); // get fortnight day [0-15]
    long shanyat = 0;
    long sya[] = {8, 8, 2, 2, 9, 3, 3, 5, 1, 4, 7, 4};
    if (mf == sya[mm - 1])
        shanyat = 1;
    return shanyat;
}
...
inline std::vector<std::string> ceMmDateTime::cal_holiday(long jdn) {
    // jdn=(long)round(jdn);
    long myt, my, mm, md, mp, mmt, gy, gm, gd, gh, gn;
    double gs;
    ceMmDateTime::j2m(jdn, myt, my, mm, md);
    ...
    atn = (long)round(ja);
    if (jdn == (atn + 1)) {
        hs.push_back("Myanmar New Year Day");
    }
    if ((my + mmt) >= BGNTG) {
        if (jdn == atn) {
            hs.push_back("Thingyan Atat");
        } else if ((jdn > akn) && (jdn < atn)) {
            hs.push_back("Thingyan Akyat");
        } else if (jdn == akn) {
            hs.push_back("Thingyan Akya");
        } else if (jdn == (akn - 1)) {
            hs.push_back("Thingyan Akyo");
        } else if (((my + mmt) >= 1369) && ((my + mmt) < 1379) &&
                   ((jdn == (akn - 2)) || ((jdn >= (atn + 2)) && (jdn <= (akn + 7))))) {
            hs.push_back("Holiday");
        }
    }
    //---------------------------------
    // holidays on gregorian calendar
    if ((gy >= 2018) && (gm == 1) && (gd == 1)) {
        hs.push_back("New Year Day");
    } else if ((gy >= 1948) && (gm == 1) && (gd == 4)) {
        hs.push_back("Independence Day");
    } else if ((gy >= 1947) && (gm == 2) && (gd == 12)) {
        hs.push_back("Union Day");
    } else if ((gy >= 1958) && (gm == 3) && (gd == 2)) {
        hs.push_back("Peasants Day");
    } else if ((gy >= 1945) && (gm == 3) && (gd == 27)) {
        hs.push_back("Resistance Day");
    } else if ((gy >= 1923) && (gm == 5) && (gd == 1)) {
        hs.push_back("Labour Day");
    } else if ((gy >= 1947) && (gm == 7) && (gd == 19)) {
        hs.push_back("Martyrs Day");
    } else if ((gy >= 1752) && (gm == 12) && (gd == 25)) {
        hs.push_back("Christmas Day");
    ...
    if ((mm == 2) && (mp == 1)) {
        hs.push_back("Buddha Day");
    } // Vesak day
    else if ((mm == 4) && (mp == 1)) {
        hs.push_back("Start of Buddhist Lent");
    } // Warso day
    else if ((mm == 7) && (mp == 1)) {
        hs.push_back("End of Buddhist Lent");
    } else if ((my >= 1379) && (mm == 7) && (md == 14 || md == 16)) {
        hs.push_back("Holiday");
    } else if ((mm == 8) && (mp == 1)) {
        hs.push_back("Tazaungdaing");
    } else if ((my >= 1379) && (mm == 8) && (md == 14)) {
        hs.push_back("Holiday");
    } else if ((my >= 1282) && (mm == 8) && (md == 25)) {
        hs.push_back("National Day");
    } else if ((mm == 10) && (md == 1)) {
        hs.push_back("Karen New Year Day");
    } else if ((mm == 12) && (mp == 1)) {
        hs.push_back("Tabaung Pwe");
    }
    //---------------------------------
    // //other holidays
...
inline std::vector<std::string> ceMmDateTime::cal_holiday2(long jdn) {
    ...
    if ((gy <= 2017) && (gm == 1) && (gd == 1)) {
        ...
        hs.push_back("G. Aung San BD");
    } else if ((gy >= 1969) && (gm == 2) && (gd == 14)) {
        hs.push_back("Valentines Day");
    } else if ((gy >= 1970) && (gm == 4) && (gd == 22)) {
        hs.push_back("Earth Day");
    } else if ((gy >= 1392) && (gm == 4) && (gd == 1)) {
        hs.push_back("April Fools Day");
    } else if ((gy >= 1948) && (gm == 5) && (gd == 8)) {
        hs.push_back("Red Cross Day");
    } else if ((gy >= 1994) && (gm == 10) && (gd == 5)) {
        hs.push_back("World Teachers Day");
    } else if ((gy >= 1947) && (gm == 10) && (gd == 24)) {
        hs.push_back("United Nations Day");
    } else if ((gy >= 1753) && (gm == 10) && (gd == 31)) {
        hs.push_back("Halloween");
    }
    if ((gy >= 1876) && (jdn == doe)) {
        hs.push_back("Easter");
    } else if ((gy >= 1876) && (jdn == (doe - 2))) {
        hs.push_back("Good Friday");
    }
    //---------------------------------
    // holidays on myanmar calendar
    if ((my >= 1309) && (mm == 11) && (md == 16)) {
        hs.push_back("Mon National Day");
    } // the ancient founding of Hanthawady
    else if ((mm == 9) && (md == 1)) {
        hs.push_back("Shan New Year Day");
        if (my >= 1306) {
            hs.push_back("Authors Day");
        }
    } // Nadaw waxing moon 1
    else if ((mm == 3) && (mp == 1)) {
        hs.push_back("Mahathamaya Day");
    } // Nayon full moon
    else if ((mm == 6) && (mp == 1)) {
        hs.push_back("Garudhamma Day");
    } // Tawthalin full moon
    else if ((my >= 1356) && (mm == 10) && (mp == 1)) {
        hs.push_back("Mothers Day");
    } // Pyatho full moon
    else if ((my >= 1370) && (mm == 12) && (mp == 1)) {
        hs.push_back("Fathers Day");
    } // Tabaung full moon
    else if ((mm == 5) && (mp == 1)) {
        hs.push_back("Metta Day");
    } // Waguang full moon
    else if ((mm == 5) && (md == 10)) {
        hs.push_back("Taungpyone Pwe");
    } // Taung Pyone Pwe
    else if ((mm == 5) && (md == 23)) {
        hs.push_back("Yadanagu Pwe");
    } // Yadanagu Pwe
    //----------------------------------------------------------------------------
    // //other holidays
...
inline std::string ceMmDateTime::j2ms(double jd, std::string fs, double tz) {
    ...
    std::string mma[] = {"First Waso", "Tagu",  "Kason",  "Nayon",   "Waso",    "Wagaung",   "Tawthalin", "Thadingyut",
...
//		1 = Sasana year starts on the first day of Tagu
//		2 = Sasana year starts on Kason full moon day
inline long ceMmDateTime::syt() { return this->_syt; }

inline void ceMmDateTime::syt(long t) { this->_syt = t; }

// Sasana year
inline long ceMmDateTime::sy() { return ceMmDateTime::my2sy(this->my(), this->mm(), this->md(), this->syt()); }
...
inline std::string ceMmDateTime::my_name() {
    ...
    std::string yna[] = {"Hpusha",  "Magha",   "Phalguni",   "Chitra", "Visakha",  "Jyeshtha",
...
// Fortnight day [1-15]
inline long ceMmDateTime::mf() { return ceMmDateTime::cal_mf(this->md()); }
...
inline std::vector<std::string> ceMmDateTime::astro() { return ceMmDateTime::cal_astro(this->jdnl()); }

// get the array of public holidays
inline std::vector<std::string> ceMmDateTime::holidays() { return ceMmDateTime::cal_holiday(this->jdnl()); }

// get the array of other holidays
inline std::vector<std::string> ceMmDateTime::holidays2() { return ceMmDateTime::cal_holiday2(this->jdnl()); }
