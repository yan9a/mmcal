...
var uis={ 
	...
	BY:2,EY:1500,LT:1062,UT:1379, //beginning and end of the calendar,
	//lower and upper threshold for accurate years
	Ref:0,hmo:0,//reference, hide mobile version option
	eFont:"Arial",mFont:"\"Myanmar Text\", \
	Padauk,\"Myanmar MN\",\"Droid Sans\",Myanmar3,Tharlon,Arial"
};
var g_jd=w2j(uis.cy,uis.cm,uis.cd,uis.Type); var g_M=j2m(g_jd,uis.Ref);
uis.y=uis.cy=g_M.my; uis.m=uis.cm=g_M.mm+12*g_M.mmt; uis.cd=g_M.md;
//----------------------------------------------------------------------------
function mSelector(my,mm,rf){
  ...
  var M1=j2m(j1,rf); var M2=j2m(j2,rf); var si=M1.mm+12*M1.mmt;
...
function UI(u){
	...
	var str=""; str+="<table class='HTable'> <tbody><tr>";
	...
	if (u.Size==1) {
		...
		str+="<td class='TdHTable TdH61'>\
			...
			class='CtrlHTable1' title='Mobile version' "+(u.Size?"":"checked")+
			" style='display:"+(u.hmo?"block":"none")+";'></td>";
		//Label for Mobile
		str+="<td class='TdHTable TdH71'>\
...
" title='Refered from books of T. N. Toe and Dr. Than Tun.'>T. N. Toe </option>\
				<!--option value='2'  "+((u.Ref==2)?"selected":"")+
					" title='Refered from J. C. Eade, Southeast Asian \
					Ephemeris, 1989.'>J. C. Eade </option> \
		...
		str+="<td class='TdHTable TdH8'>\
			<select  id='calType' class='CtrlHTable1 CtrlH8'  \
...
function initc(hmo){ uis.hmo=hmo||0; //parameter to hide mobile option
	...
	if (uis.Size===undefined) { var dw=ce.offsetWidth;
			if (dw<693)  uis.Size=0; else uis.Size=1; }	
...
function emc(v) {
	...
	var j1=Math.round(SY*yn+MO)+1; var j2=Math.round(SY*(yn+1)+MO);
        var M1=j2m(j1,uis.Ref); var M2=j2m(j2,uis.Ref);
		var si=M1.mm+12*M1.mmt; var ei=M2.mm+12*M2.mmt;
	if(mn==0) mn=(v==1)?4:3;
	else if (mn==4 && M1.myt!=0 && v!=1) mn=0;
	else if (mn==3 && M1.myt!=0 && v==1) mn=0;
	else { mn+=Number(v);
	if (mn<si) { mn+=12; yn--; } else if (mn>ei) { mn=mn%12; yn++; }}
	uis.y=yn; uis.m=mn;
    me.innerHTML=mSelector(uis.y,uis.m,uis.Ref); ye.value=yn;
...
function DisplayUI(cev) {
	switch(cev) {
		case 1: var ye = document.getElementById("y"); var yn=Number(ye.value);
			if(yn<uis.BY) {ye.value=yn=uis.BY;}
			else if(yn>uis.EY) {ye.value=yn=uis.EY;}
			else {ye.value=yn;}
			uis.y=yn;
			var me = document.getElementById("m"); uis.m=Number(me.value);
			me.innerHTML=mSelector(uis.y,uis.m,uis.Ref);
...
function UIContent(u) {
	...
	var mmt=Math.floor(u.m/13); var mm=(u.m+mmt)%13; 
	js=m2j(u.y,mm,mmt,0,1,u.Ref);
	MS=j2m(js,u.Ref); je=js+MS.mml-1; ME=j2m(je,u.Ref); 
	ES=j2w(js,u.Type); EE=j2w(je,u.Type);
	//------------------------------------------------------------------------
	//Start of the table for calendar days
	var str=" <table class='MCTable'> ";
	str+=tHead(cFont,u,MS,ME,ES,EE) ;	//header
	str+=tFoot(cFont,u);//footer
	str+="<tbody>";

	str+=tWeek(cFont,u); //Weekday header row
	...
	for(i=0;i<eml;i++){
	   ...
	   <p class='MCTdP"+u.Size+"' style='font-family: "+cFont+";'>";
		...
		if ((i>=r)&&(js<=je)) { 
			...
			tstr="MCEd"+u.Size;//Myanmar date and astrological days 
			...
			if ((M.my==u.cy) && (u.cm==(M.mm+12*M.mmt)) && (M.md==u.cd))
			...
			if (M.my>=2) {
				if (u.Size==1) str+=mMDStr(M,A); //for median calendar size					
				else str+=sMDStr(M,A.sabbath,MS.mm,u.eFont,tstr); //if small
			}//end of if (M.my>=2)

		    if (u.Size==0){ if (E.m==ES.m)  tstr="MCMd2"; else tstr="MCMd3"; }
			//set style according to month
			else {
				str+="<a href='http://mc1500.com/wiki/doku.php?id=me_"+M.my;
				str+="' class='MCMs'>"+T(ema[E.m-1])+"</a> ";
			}
			str+="<a href='http://mc1500.com/wiki/doku.php?id=me_"+M.my;
			...
			//Displaying Myanmar 
			if (M.my>=2 && u.Size==1) {
				str+=mMDStr2(M,A,E,u.Astro,hde,hdm,hdt,hdo,js); //median size					
	...
	//Foot note
	if (u.Size==1) {
		if (u.y<u.LT || u.y>u.UT)
			str+="<p class='MCFootIM'\
...
//glabal variable: ema, mma
//input: (cFont: font name, u: UI setting , MS: Myanmar date start , 
// ME: Myanmar date end, ES: English date start, EE: En date end)
//output: (str: html string)
function tHead(cFont,u,MS,ME,ES,EE) {
	var str="<thead> <tr> <th colspan=7 class='MCTh"+u.Size+"'>\
	<p class='MCThP' style='font-family: "+cFont+";'>";//table header
	 str+="<a  class='MCThA1"+u.Size+"' href='http://mc1500.com'>"+n2s(ES.y);
	 if (ES.y!=EE.y) str+="-"+n2s(EE.y)+" </a>";
	 str+=" <a  class='MCThA2"+u.Size+"' href='http://mc1500.com'>"+
		T(ema[ES.m-1])+"</a>";
	 if (ES.m!=EE.m) str+="-<a  class='MCThA3"+u.Size+
		"' href='http://mc1500.com'>"+T(ema[EE.m-1])+"</a>";
	 str+=T(',')+" ";
...
function tFoot(cFont,u) {
	...
	<p class='MCTdP"+u.Size+"' style='font-family: "+cFont+";'>";
		if (u.Size==1) {
			str+="<a class='MCAnchor' href='http://mc1500.com/'>\
			E-&gt;M Calendar</a> ";
...
// MS_mm: Myanmar month at start, u_eFont: Font name, tstr: class)
//output: (str: html string)
function sMDStr(M,A_sabbath,MS_mm,u_eFont,tstr) {
	var str="";
...
//output: (str: html string)
function mMDStr2(M,A,E,u_Astro,hde,hdm,hdt,hdo,jd) {
